...
const corsOptions = {
  allowedHeaders: ['X-Acid-Request', 'X-Requested-With'],
  exposedHeaders: ['Content-Length', 'Pragma', 'X-Powered-By', 'Vary'],
...
app.configure(rest((req, res) => {
    res.format({
      'application/acid.cors-ims-lookup.1+json': () => {
  ...
  .configure(auth({secret: 'ImACopYouIdiot'}))
  .configure(local())
  .configure(jwt())
  .use('/users', memory())
  // .use((req, res, next) => {
  ...
  // })
  .configure(services())
  .post('/login', auth.express.authenticate('local', {
