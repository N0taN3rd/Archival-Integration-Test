    ...
    axios({
      ...
      if (error.response) {
        ...
        this.setState({
          ...
          display,
          badge: (
            <div className='uk-card-badge uk-label uk-label-danger'>HTTP {error.response.status} {error.response.statusText}</div>
          )
        })
    ...
    return (
      <div className='uk-card uk-card-default uk-box-shadow-xlarge'>
        <div className='uk-card-body'>
          {this.state.badge}
          <h3 className='uk-card-title'>HEAD</h3>
          <p>Can I Make An HTTP Head Request?</p>
